    ...
    IEnumerator Spawn(int _redniBroj)
    {
        ...
        if (currentGun)
        {
            ...
            if (currentGun.name.Contains("Gun"))
            {
                ...
                if (gunName.Contains(nameof(GunNames.NewGun_auto)))
                {
                    ItemScript.Instance.bullet_auto = (int)(gunScript.bulletsIHave + gunScript.bulletsInTheGun);
                }
                else if (gunName.Contains(nameof(GunNames.NewGun_semi)))
                {
                    ItemScript.Instance.bullet_semi = (int)(gunScript.bulletsIHave + gunScript.bulletsInTheGun);
                }
                ...
                gunName = gunsIHave[_redniBroj].ToString();
                int bulletNum = 0;
                switch (gunName)
                {
                    case nameof(GunNames.NewGun_auto):
                        gunScript.gunName = nameof(GunNames.NewGun_auto);
                        bulletNum = ItemScript.Instance.bullet_auto;
                        break;
                    case nameof(GunNames.NewGun_semi):
                        gunScript.gunName = nameof(GunNames.NewGun_semi);
                        bulletNum = ItemScript.Instance.bullet_semi;
                        break;
                    default:
                        break;
                }
                if (gunScript != null)
                {
                    if (bulletNum > gunScript.amountOfBulletsPerLoad)
                    {
                        gunScript.bulletsInTheGun = gunScript.amountOfBulletsPerLoad;
                        gunScript.bulletsIHave = bulletNum - gunScript.amountOfBulletsPerLoad;
                    }
                    else
                    {
                        gunScript.bulletsInTheGun = bulletNum;
                        gunScript.bulletsIHave = 0;
                    }
                }
        ...
        else
        {
            ...
            // 重新获取对象脚本
            GunScript gunScript = currentGun.GetComponent<GunScript>();
            string gunName = gunsIHave[_redniBroj].ToString();
            int bulletNum = 0;
            // 计算对应子弹数
            switch (gunName)
            {
