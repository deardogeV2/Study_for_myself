...
public class PlayerMovementScript : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (TopCamera!=null) TopCamera.SetActive(false);
    ...
    public float intervalPickUp = 1f;
    /*
	...
	*/
    ...
    private ItemInfo pickItem;
    void PlayPickUp()
    {
        // 在间隔时间CD时并且按下F进行拾取
        Ray ray = Camera.main.ScreenPointToRay(new Vector2(Screen.width/2, Screen.height/2));
        Debug.DrawRay(ray.origin, ray.direction * pickUpDistance, Color.black,2f);

        if (Time.time - lastPickUpTime > intervalPickUp)
        {
            if (Input.GetKeyDown(KeyCode.H))
            {
                RaycastHit[] PlayerLookHits = Physics.RaycastAll(ray, pickUpDistance);

                if (PlayerLookHits.Length>0)
                {
                    curWeapon = GameObject.FindGameObjectWithTag("Weapon").GetComponent<GunScript>();
                    string weaponName = "";
                    if (curWeapon != null)
                    {
                        weaponName = curWeapon.gunName;
                    }
                    foreach (RaycastHit playerLookHit in PlayerLookHits)
                    {
                        if (playerLookHit.collider != null && playerLookHit.collider.transform.tag == "Item")
                        {
                            pickItem = playerLookHit.collider.transform.GetComponent<ItemInfo>();
                            if (playerLookHit.collider.gameObject.name.Contains("bulletAuto"))
                            {
                                if (weaponName.Contains(nameof(GunInventory.GunNames.NewGun_auto)))
                                {
                                    curWeapon.bulletsIHave += pickItem.Number;
                                }
                                else
                                {
                                    ItemScript.Instance.bullet_auto += pickItem.Number;
                                }
                            }
                            else if (playerLookHit.collider.gameObject.name.Contains("bulletSemi"))
                            {
                                if (weaponName.Contains(nameof(GunInventory.GunNames.NewGun_semi)))
                                {
                                    curWeapon.bulletsIHave += pickItem.Number;
                                }
                                else
                                {
                                    ItemScript.Instance.bullet_semi += pickItem.Number;
                                }
                            }

                            //播放音频
                            if (ItemGetSound!=null) ItemGetSound.Play();
                            // 对象销毁逻辑
                            Destroy(playerLookHit.collider.gameObject);
                            // 额外销毁地图对象中的列表
                            TaskManager taskManager = GameObject.Find("Map").GetComponent<TaskManager>();
                            taskManager.items.Remove(playerLookHit.collider.transform);
                        }
                    }
                }
            }
