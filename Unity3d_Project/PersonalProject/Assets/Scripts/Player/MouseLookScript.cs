...
public class MouseLookScript : MonoBehaviour {
	...
	void Awake(){
		...
		PlayerBottom = GameObject.Find("Bottom").transform;
	}

	public float zRotation;
	public float timeTotoCrooked = 15f;
  ...
  //      {
		...
		//	wantedZ = Input.GetKey(KeyCode.Q) ? 15f : -15f;

		//	zRotation = Mathf.Lerp(zRotation, wantedZ, Time.deltaTime* timeTotoCrooked);
		//}
...
void ApplyingStuff(){
		...
		if (AngleZ >= 345) AngleZ = AngleZ - 360;
		float WantAngleZ = 0f;
		if (Input.GetKey(KeyCode.Q) && Input.GetKey(KeyCode.E)) WantAngleZ = 0f;
		else if (Input.GetKey(KeyCode.Q)) WantAngleZ = 15f;
		else if (Input.GetKey(KeyCode.E)) WantAngleZ = -15f;

		float nowAngleZ = Mathf.Lerp(AngleZ, WantAngleZ, Time.deltaTime * 15f);
		transform.rotation = Quaternion.Euler(0, currentYRotation, nowAngleZ);
