...
SOUR_PATH = r'''trunk\issue\互联网医院\202009\28\physicial_examination_h5.zip'''
# =========================================================================
'''
这里稍微记录一下Git我常用的代码：
git add 文件 ：添加待处理文件
git status ：查看现在的待处理文件
git add .：将本地仓库中的更新（不包括删除）全部添加为待处理文件
git add --a:将本地仓库中的更新（包括删除）全部添加为待处理文件
git commit -m 'log'：将待处理文件上传并且提交log
git push：将本地仓库提交的修改pash到远端仓库
git pull：从远端仓库提取最新代码
git rm -r --cached filename：删除待处理的文件
git rm 文件：删除本地仓库文件，如果提交并且push可以实现删除远端文件
git branch -a:查看全部分支
